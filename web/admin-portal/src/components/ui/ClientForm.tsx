...
export default function ClientForm({
  ...
  const buildShareMessage = (url: string) =>
    `Здравствуйте!\nС начала сентября для групповых занятий вводится несколько изменений -  подробнее  https://t.me/c/2059721358/4404 ☘️ \n\n${url} - это Ваша карточка клиента! 🐸\nЗдесь информация о типе посещения, при наличии абонемента срок действия, остаток, а также уникальный и обязательный qr-код для входа! \n\nИнструкция: на ресепшене будет планшет с открытым приложением и окошком камеры для сканирования - подставляете свой телефон с открытым qr-кодом, и приложение автоматически учитывает ваш проход в личной карточке ✅ \n\nПожалуйста, проверьте и напишите в ответ, все ли данные верны☘️\nСпасибо за доверие!`;

  const copyClientInfo = () => {
    if (!passUrl) return;
    copyToClipboard(buildShareMessage(passUrl));
  };
